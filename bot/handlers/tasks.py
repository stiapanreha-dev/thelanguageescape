...
async def callback_answer_task(callback: CallbackQuery, session: AsyncSession, state: FSMContext):
    ...
    if is_correct:
        ...
        if task_number < total_tasks:
            ...
            # 1. If next task is voice/text_input - always auto-transition
            #    (these types handle their own auto-transition internally)
            #    Note: audio is NOT included - user must answer the question first
            ...
            if next_task and next_task.get('type') in ['voice', 'text_input']:
